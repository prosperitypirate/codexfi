#!/usr/bin/env bun
/**
 * CLI entry point — `opencode-memory <command> [options]`.
 *
...
function printHelp(): void {
	...
	fmt.blank();
	console.log(`    ${fmt.dim("$")} opencode-memory install`);
	console.log(`    ${fmt.dim("$")} opencode-memory list --type architecture`);
	console.log(`    ${fmt.dim("$")} opencode-memory search "how is auth handled"`);
	console.log(`    ${fmt.dim("$")} opencode-memory stats`);
	console.log(`    ${fmt.dim("$")} opencode-memory export --format csv --output backup.csv`);
	fmt.blank();
}
...
async function main(): Promise<void> {
	...
	if (!cmd) {
		...
		fmt.blank();
		fmt.info(`Run ${fmt.cyan("opencode-memory help")} to see available commands.`);
		fmt.blank();
		process.exit(1);
	}
