/**
 * `opencode-memory forget <id>` — delete a memory by ID.
 *
 * Permanently removes a memory from the LanceDB store.
 * Use `opencode-memory list` to find memory IDs.
 *
...
export async function run(args: ParsedArgs): Promise<void> {
	...
	if (!idInput) {
		...
		fmt.blank();
		fmt.info(`Usage: ${fmt.cyan("opencode-memory forget")} ${fmt.dim("<id>")}`);
		fmt.info(`Run ${fmt.cyan("opencode-memory list")} to see memory IDs.`);
		process.exit(1);
	}
