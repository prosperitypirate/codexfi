/**
 * `opencode-memory status` — health check for the memory system.
 *
 * Verifies:
	...
	try {
		...
		for (const filename of candidates) {
			...
			const hasMemory = plugins.some(
				(p) => p.includes("opencode-memory") || p.includes("plugin-v2"),
			);
...
function checkLogFile(): CheckResult {
	const name = "Log file";
	const logPath = join(homedir(), ".opencode-memory.log");
