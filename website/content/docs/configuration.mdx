...
All memory data is stored locally at `~/.codexfi/store.jsonl`. This is a pure TypeScript vector store — no native dependencies, no external process required.

| Path | Contents |
|------|----------|
| `~/.codexfi/store.jsonl` | Vector store (all memories with embeddings) |
