...
in the background. It uses a pure TypeScript vector store + Voyage AI embeddings,
running locally.
...
The `codexfi install` command creates a `/memory-init` slash command. Use it when you want thorough structured memory right away rather than letting it build up naturally over sessions:
...
- Memory data is stored at `~/.codexfi/store.jsonl` — the store is yours to inspect or delete
